...
15. Tomas may put process instructions for the assistant in standalone `<...>` blocks while drafting. Treat these as private authoring instructions, not public prose, unless he explicitly asks to keep them. Do not apply this rule to normal HTML, code fences, generics, or inline comparison operators.
16. Draft articles should use `published: false` in front matter and must not be added to `interactive\article-index.json` until Tomas wants them published. For styled local draft review, keep a generated snapshot and use `python tools\generate_interactive_site.py --preview-drafts`; normal generator runs remove draft pages from `_site`. Publishing means removing private authoring notes, setting or omitting `published: true`, adding the index entry, and generating the durable HTML/source/caveman snapshot.
17. Tomas may use standalone `<image Name>` instructions while drafting. Resolve these as screenshots/images expected under the article year in `images\YYYY\` and insert normal Markdown image syntax with a useful Czech alt text. If the file is not present yet, use the expected path and note that the image file still needs to be supplied; do not treat the `<image ...>` marker as public prose.
