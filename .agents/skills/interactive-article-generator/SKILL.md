...
English machine translations live under `/en/`. Every published Czech interactive article must have a current English translation entry and generated snapshot; draft sources with `published: false` are the only exception. For translated source `interactive\translations\en\YYYY\slug.article.md`, create and commit:
...
- Do not pre-compose duplicate back/CZ/EN links in generated snapshots. The site generator normalizes `.ia-links`; validation fails if an article nav contains duplicate language links, duplicate `source.md`/`caveman.md` hrefs, or duplicate `aria-label`.
...
- For every published Czech article, `interactive\article-index.en.json`, `interactive\translations\en\YYYY\slug.article.md`, and `interactive\generated\en\YYYY\slug\{index.html,source.md,caveman.md}` exist and use the current Czech source hash.
...
- Article `.ia-links` navigation contains each required href exactly once: back, current/other language where applicable, `./source.md`, and `./caveman.md`.
